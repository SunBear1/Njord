...
export const INPUT_CLS =
  'w-full border border-border rounded-lg px-3 py-2 text-sm bg-bg-primary text-text-primary ' +
  'focus:outline-none focus:ring-2 focus:ring-accent-primary/30 ' +
  'placeholder:text-text-muted';
export const LABEL_CLS = 'text-xs font-medium text-text-secondary';
...
export function fmtGain(gain: number): { text: string; cls: string } {
  if (gain > 0) return { text: `+${fmtPLNGrosze(gain)}`, cls: 'text-success' };
  if (gain < 0) return { text: fmtPLNGrosze(gain), cls: 'text-danger' };
  return { text: fmtPLNGrosze(0), cls: 'text-text-muted' };
}
