...
const SEED_SQL_PATH = resolve(process.cwd(), 'infrastructure/seed-nbp-forecasts.sql');
...
async function main(): Promise<void> {
  try {
    ...
    await writeSummary([
      '## 📈 NBP Forecast Seed Apply Results',
      '',
      `| Metric | Count |`,
      `|--------|-------|`,
      `| Seed file | infrastructure/seed-nbp-forecasts.sql |`,
      `| Statements found | ${statements.length} |`,
      `| Statements executed | ${executed} |`,
      `| Rows inserted or replaced | ${changedRows} |`,
    ].join('\n'));

    console.log('Done.');
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    await writeSummary(`## ❌ NBP Forecast Seed Apply Failed\n\n${message}`);
